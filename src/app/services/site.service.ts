...
export class SiteService {
  ...
  // branches
  getbranches(){
    return this.http.get(getBranchesUrl).pipe(map((res: GeneralResponse)=> res.data));
  }
  ...
  // services 
  getServices(){
    return this.http.get(getServicesUrl).pipe(map((res: GeneralResponse)=> res.data));
  }
  ...
  // sliders   
  getSliders(){
    return this.http.get(getSlidersUrl).pipe(map((res: {data} )=> res.data));
  }
  ...
  updateSlider(id, slider){
    return this.http.post(updateSliderUrl + id, slider).pipe(map((res: {data} )=> res.data), take(1));
  }
  ...
  // contacts  
  getContacts(){
    return this.http.get(getContactsUrl).pipe(map((res: {data} )=> res.data));
  }
